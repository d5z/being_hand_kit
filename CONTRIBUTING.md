...
**运输层：回执恒绿，只有落点回读抓得住（0.9.4 后续，axiang/iCat 样本）**

多行内容跨壳运输是高危面——axiang 的样本：`.cmd` 脚本在首个换行处静默截断，exit 0 无报错；iCat 的样本：35 行飞书消息只送达第一行，回执 `ok:true`。两枚归一族：**运输层吃掉内容时，上游回执恒绿**。L2 证明代码发对了调用，live 证明世界接住了，运输层可以在两层都绿的情况下吃掉内容。

判据（按判据写，不按步骤写——步骤会被下一个人抄到病发线上）：

**回读点与被检点在依赖图上越远（空间）、时间上越近（当轮），这个检查越硬。**

两个维度缺一不可：

- **空间维**：回读路径与被检路径共享的组件越少，检查越可信。具体两枚钉子——①比对基准（摘要/指纹）不能走被检的同一条路：基准在入壳前算好，用不会截断的信道带过去（纯标量 hex，不拼进多行正文——否则 corrupted == corrupted 照样绿）；②回读要读「落下的」不是「发出的」：回读口必须是消费者真会走的那条（消息服务 GET 一条），不是本地日志或发送缓冲的回显。
- **时间维**：回读必须落在当轮，不跨缓冲、不跨消化窗、不等批处理。反面样本（axiang）：延后窗口里状态被改写/归一，回读出来是修好的样子，绿得毫无破绽——「当轮」不是性能优化，是正确性要求。

附注（三处工地反读）：Hand（工具层）、记忆写入探针（Sponza，当轮反向读）、消息证据链（Reki）三处独立长出同形判据——但共享判据不等于互相独立验证：三处若都默认「有回执即送达」，就是同一个朴素假设的三份拷贝，一致地错。要证独立，各亮一枚本层特有的失败样本（换行截断 / 消化窗改写 / 链接失活）——样本不同证视角不同，判据相同证可迁移，两件事得同时成立。
