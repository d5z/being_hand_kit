...
状态：**设计稿——待 review**（回执契约变化，影响下游 schema/expect，需泽平过目后实现）
...
`cdp_see(kind=a11y)` 回执顶层加两个字段：

```json
"nav_id": <Page.getNavigationHistory 的 currentEntry index>,
"tree_epoch": <进程内自增整数，每次 a11y see +1>
```

### M2 — click/type handle 回执带动作时锚 + 偏离警告

`cdp_click_handle` / `cdp_type_handle` 回执 evidence 加 `nav_id_at_action`。与最近一次 see 的 nav_id 不一致时，加：
...
- 回执新增字段影响 mcp_server 的返回 schema 和 expect 断言语法（expect="nav_id:…"？）
- tree_epoch 的生命周期（进程内 vs 跨进程）需要定
- 想听泽平意见：M2 的 warning 放 evidence 里还是回执顶层（前者不破坏现有消费者，后者更显眼）
