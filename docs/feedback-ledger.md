...
| F23 | **macOS 系统代理吃掉 CDP 回环流量（探针瞎了→误杀活 Chrome）**：urllib 默认 opener 读系统代理（scutil --proxy：HTTP/HTTPS/SOCKS 全开 127.0.0.1:6789，Clash 类常见配置），把 localhost:9222 请求送进代理，代理对回环流量回 502。证据三连（同机同活 Chrome）：curl 9222→200；urllib 默认 opener→502；urllib 空 opener（ProxyHandler({})）→200。scutil ExceptionsList 虽含 localhost 但 urllib 读代理时没吃到例外。**受影响面三处**：cdp_launcher.endpoint_info()（探针瞎→cdp_open 误杀健康 Chrome 循环重启）、open_new_tab()、cdp_core._json_get()（see/nav/click 全家）——macOS+系统代理机器上 CDP 全系不可用。**9/6「9222 十秒不可达」旧案根因落定**（当时怀疑 IPv6 未验证，同症状真根因是代理）。修法：CDP_HOST 恒回环的三处 urlopen 换无代理 opener；vision_llm.py 和 Grove 心跳走外网保留系统代理不动 | 216（#34 seq 1718，0.9.5 升级后实机根因样本，本地补丁已实测通过 5 处编辑） | **P1**（macOS 采用面） | 0.9.6 主案候选——已回帖请 216 贴 diff（#34 1719），按 taojun 0.9.5 同款流程走 |
| F24 | **Windows 安装器两坑**（taojun 0.9.5 升级实测）：① bundle 实为 tar.gz，存成 .zip 后 Expand-Archive 报「找不到中央目录结尾记录」——报错不提示「可能不是 zip」，有误导性。修法方向：按 magic bytes 判格式再解，或下载时直接存 .tar.gz 后缀。② `Set-Content -Encoding UTF8` 写 manifest 必带 BOM（EF BB BF 实测），`[IO.File]::WriteAllText + UTF8Encoding(false)` 才干净——与 0.9.5 修的读侧容 BOM 是同一族两半：读侧已容，写侧（若做安装器）从源头不产 | taojun（DM 09-25 08:13，F16 收尾顺带） | P3（若做 Windows 安装器才触发） | 0.9.6 候选 |
