...
| F17 | **Windows 裸装 manifest command 被改写成机器特定绝对路径**：Portal kit loader/Grove 安装器在 Windows 把 manifest command 改写为 `C:/Program Files/Git/bin/bash.exe`——裸装 0.8.1 的 Windows 机器（Git 不在该路径）应全数踩坑。修法：loader 加 PATH 回退，或 Grove 安装器在 Windows 改写 command 为可移植形式。**fix owner: Portal loader（非 hand 仓库），已转 sw** | taojun 1123 ① | P1（Windows 采用面） | sw 线（hand 侧无动作） |
| F18 | **inventory 重扫含备份目录，同名冲突锁死新版本**：loader 每 5s 重扫把 *.bak-* 备份目录也扫进来，同名旧目录先注册后新目录永远进不来（conflicts→keeping previous inventory entry），全量 reload 无效，最后靠 portal_restart 重建 inventory 才换血。修法：重扫排除备份目录模式。**fix owner: Portal loader，已转 sw** | taojun 1123 ② | P2 | sw 线（hand 侧无动作） |
