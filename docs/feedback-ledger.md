...
| F16 | **CDP_PORT 硬编码 9222**：launcher-embedded Chrome（taojun 的 D5 Launcher 内嵌 9222）占用端口时，kit spawn 撞车——更险的形态是静默错连（cdp_connect 打到占用者的 Chrome 上，不是 kit 自己 spawn 的）。taojun 本地补丁走 9223 避开。修法：`HAND_CDP_PORT` env 覆盖（1123 定名，0.8.2 落线；动态选空闲端口方案未采纳） | taojun 1116（9/21 6.11.0 本地补丁核对 F7/F8 覆盖范围时暴露：三项里 profile/有头已被 0.7.0 原生覆盖，唯端口未被覆盖） | P2 | **✅ 已修 0.8.2**（原 0.9 候选提前落线；1123 定名 `HAND_CDP_PORT`，默认 9222 不变——taojun 本机实测 9223/9222 并存互不干扰，0.8.1 静默寄生 D5 Launcher 内嵌 Chrome 的实锤（连进 dl://launcher/docker.html，Chrome/134 无登录态）。收案进行时（09-22 1260）：0.9.0 原地升级自动冲掉本地补丁（回干净主干），kit .env 幸存、9223 在岗（kit 进程 13:42 晚于 .env 10:35 写入）；taojun 加 User env 双保险并冻结 cdp_open，待自然使用 netstat 实证 9223 监听后回报——最后一笔待收** |
| F17 | **Windows 安装层「no loadable manifest」根因=备份目录同名冲突（与 F18 同根，合并定论）**：`Kit 'hand' has no loadable manifest`（mod.rs:226）触发链=scan_kits_from_dir 防劫持机制把同名路由冲突的目录**全部剔除**——kits\hand + hand.bak-6.9.0 + hand.bak-6.11.0 三方 manifest name 都是 "hand" → 冲突 → 全剔除 → target 找不到。manifest.json 本身合法无关。taojun 改写 command 为绝对路径**不是起效变量**：源码显示 command 解析失败只产生 warning（manifest_validation_warnings），kit 照样进列表。修法（portal repo）：①报错带冲突详情（现在只报下游症状）；②scan 跳过 .bak 后缀或冲突时保留最新 mtime。**fix owner: Portal loader，已转 sw + 源码定论已交付** | taojun 1123① + 1139①②③ + 源码钉死（loader.rs scan 尾部 conflicts 逻辑） | P1（Windows 采用面） | 已收案（1141）——修复在 portal repo，等 sw 拍板 |
| F18 | **inventory 重扫含备份目录，同名冲突锁死新版本（=F17 根因本体）**：loader 每 5s 重扫把 *.bak-* 备份目录也扫进来，同名冲突时「keeping previous inventory entry」全量 reload 无效，最后靠 portal_restart 重建 inventory 才换血。与 F17 合并：冲突剔除机制是「no loadable manifest」的直接上游。修法：重扫排除备份目录模式 / 冲突报错带详情。**fix owner: Portal loader，已转 sw** | taojun 1123② | P2 | 已收案（与 F17 合并，1141）——修复在 portal repo，等 sw 拍板 |
...
| F22 | **headless 下原生 `<select>` 不可操作 + cdp_type 落点不验证（假 ok 新变体）**（Alice dogfood 2026-09-22，双实验升格）：①option 在 AX 树可见但 not in render tree → click 诚实报错 "element has no box"（这部分是声明的失败，不是坑）。②cdp_type 对 focused select 无效：`Input.insertText` 不走 `document.activeElement`，走浏览器 text-input context——有旧可编辑目标时**落点漂移**（exp1：搜索框 "BOM"→"BOM已修复"），无旧目标时**静默丢弃**（exp2：21 条全显示，文本零落地）。③回执缺口：两例回执均 `ok + verified`，verified 验的是 focus 不是落点——exp2 是干净的「回执 ok 但文本零落地」。修法方向：cdp_type 加 post-flight read-back（对齐 click 的 read_back 语义）；非可编辑 tag（SELECT 等）直接诚实拒绝或声明漂移。原生 select 在 headless 下经 hand 全路径不可操作（click 无 box / type 无 typeahead / 无键盘事件工具）——测试目标页应避免原生 select，或走页面自身 API | Alice dogfood 双实验（exp1 污染样本 + exp2 干净复现） | P2（回执诚实主线） | **✅ 已修待发**（Judy f22-type-readback 三路径全接：pre-flight 可编辑性拒绝 + post-flight read-back + Enter 前回读；矩阵 9/9 + 415 回归全绿，判读通过合并 d80cde0，随 0.9.7 发） |
| F23 | **macOS 系统代理吃掉 CDP 回环流量（探针瞎了→误杀活 Chrome）**：urllib 默认 opener 读系统代理（scutil --proxy：HTTP/HTTPS/SOCKS 全开 127.0.0.1:6789，Clash 类常见配置），把 localhost:9222 请求送进代理，代理对回环流量回 502。证据三连（同机同活 Chrome）：curl 9222→200；urllib 默认 opener→502；urllib 空 opener（ProxyHandler({})）→200。scutil ExceptionsList 虽含 localhost 但 urllib 读代理时没吃到例外。**受影响面三处**：cdp_launcher.endpoint_info()（探针瞎→cdp_open 误杀健康 Chrome 循环重启）、open_new_tab()、cdp_core._json_get()（see/nav/click 全家）——macOS+系统代理机器上 CDP 全系不可用。**9/6「9222 十秒不可达」旧案根因落定**（当时怀疑 IPv6 未验证，同症状真根因是代理）。修法：CDP_HOST 恒回环的三处 urlopen 换无代理 opener；vision_llm.py 和 Grove 心跳走外网保留系统代理不动 | 216（#34 seq 1718/1721，0.9.5 升级后实机根因样本，本地补丁已实测通过 5 处编辑） | **P1**（macOS 采用面） | **✅ 已修 0.9.6**（commit 6d09798，模块级 opener；Linux 侧测试调和：probe mock 目标 urlopen→opener.open，mock 失效会静默直连真 Chrome；**216 实机回执已收** #34 seq 1732：撤补丁升官方 0.9.6、diff 零差异、系统代理全开下原故障场景复测通过） |
| F24 | **Windows 安装器两坑**（taojun 0.9.5 升级实测）：① bundle 实为 tar.gz，存成 .zip 后 Expand-Archive 报「找不到中央目录结尾记录」——报错不提示「可能不是 zip」，有误导性。修法方向：按 magic bytes 判格式再解，或下载时直接存 .tar.gz 后缀。② `Set-Content -Encoding UTF8` 写 manifest 必带 BOM（EF BB BF 实测），`[IO.File]::WriteAllText + UTF8Encoding(false)` 才干净——与 0.9.5 修的读侧容 BOM 是同一族两半：读侧已容，写侧（若做安装器）从源头不产 | taojun（DM 09-25 08:13，F16 收尾顺带） | P3（若做 Windows 安装器才触发） | 未排期——设计输入，做安装器时进 |
| F25 | **rsync --delete 同步 bundle 会删掉本机安装痕迹**（216 0.9.6 升级实测）：用 `rsync --delete` 把官方 bundle 同步进 kit 目录时，bundle 里没有的本机文件被当多余删掉——216 的 `.venv -> venv` 软链被删，start.sh 找不到 .venv 回落系统 python3（无 mcp），kit 起不来报 mcp-start-failed。修法：重建软链即活。教训：同步前先盘点「bundle 之外的本机安装痕迹」（.venv 软链、.env、PATCHES.md），同步时显式 exclude——机器本地 artifact 和本地补丁是两类东西，撤补丁不能顺带把安装痕迹也撤了 | 216（#34 seq 1732，0.9.6 实机回执附带） | P3（升级文档） | 0.9.7 升级文档收 |
| F26 | **kits/ 下杂散 manifest 目录 → reload 报 no loadable manifest**（Judy 0.9.6 升级实测）：`~/.heart-portal/kits/` 下带 manifest.json 的杂散目录（她 mv 旧版的 .bak 残留）导致 portal_kits_reload 精确复现故障。单因子实验锁定：A（权限全脏 600/700 + 无杂散目录 → reload 成功）洗清权限，B（权限全净 + 仅摆一个杂散 manifest 目录 → 复现故障）锁定真凶。600/700 外观本身 = umask 0077 + 裸 `tar xzf`（无害，download 端点字节干净三源可比）。修法/SOP：版本备份放 kits/ 之外（~/hand-kit-{ver}-bak-{时间戳}）；解压目标路径是 `~/.heart-portal/kits/hand/`（bundle 根就是 kit 目录本身，hand/ 是 python 包不是外层目录名） | Judy（#34 seq 1736/1744，单因子实验 1744，收案 1752/1759） | P3（升级文档） | 0.9.7 升级文档收 |
