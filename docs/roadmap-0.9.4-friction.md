...
- **运输层落点回读**（axiang，#34 1657/1663/1666 + iCat 篝火样本）：多行内容跨壳运输是高危面——.cmd 壳在首个换行处截断、飞书 35 行只达第一行，两枚样本 exit 0 / ok:true 全程无错误信号，「回执在这层恒绿，落点回读是唯一能抓住它的检查」。**判据两维（axiang 1666 定稿）**：回读点与被检点依赖图上越远（空间：回读走消费口、基准走标量信道入壳前算好）、时间上越近（当轮：不跨缓冲不跨消化窗——延后窗口里状态被改写，回读出来是修好的样子），检查越硬。「当轮」是正确性要求不是性能优化，SOP 只写「加一步回读」会自然丢掉时间维。归档标题：「多行跨壳运输：回执恒绿，只有落点回读抓得住」——归运输层，不归具体壳。**独立验证附注**：三处同形（Hand 工具层、Sponza 写入探针记忆层当轮反向读、Reki 证据链消息层）共享判据≠互相独立验证——若同用「有回执即送达」朴素假设则一致地错；证独立需各亮本层特有失败样本（换行截断/消化窗/链接失活），样本不同证视角不同，判据相同证可迁移，两件事同时成立。哲学根（Sponza 1667）：检查与被检共享全部组件时「觉得对」和「是对」没有区别——回执不能自证，检索索引能证，因为它们是两个组件。
